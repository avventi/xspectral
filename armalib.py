...
def pp2p(q,acc=1e-6,max_iter=100):
	""" Given the coefficients of the pseudopolynimial
		q(z) = q_0 + q_1 (z+z^-1) + ... q_n (z^n + z^-n),
	this function returns the coefficients of the unique, if any, strictly 
	minimum-phase polynomial p(z) such that q(z) = p(z) p(z^-1)."""
	n = len(q)
	q_act = ones_like(q)
	theta = q / sqrt(q[0])
	unstable = False
...
def arma2cep(num,den,N):
	""" Evaluates the cepstral coefficients up to the N-th order of the process
	generated by feeding with white noise the ARMA filter G(z) = num(z)/den(z).""" 
	n = len(den) -1
	m = len(num) -1
...
def arma2cov(num,den,N):
	""" Evaluates the covariance coefficients up to the N-th order of the process
	generated by feeding with white noise the ARMA filter G(z) = num(z)/den(z)."""
	n = len(den) -1
	m = len(num) -1
	# pad zeroes if necessary
